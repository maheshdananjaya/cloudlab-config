...
bool DTX::LatchLog() {
  ...
  //TODO- add last flag. object should be marked for the last flag during execution. ]~
...
void DTX::Abort(coro_yield_t& yield) {
  ...
  *((lock_t*)unlock_buf) = 0;
  for (auto& index : locked_rw_set) {
    ...
    RDMA_LOG(INFO) << "Send Unlock "<< t_id;
  ...
  RDMA_LOG(WARNING) << "Suceesfully Aborted  " << t_id ; //d << " key and table " << it->key << "  " << it->table_id;
}
