  ...
  for (auto& re : pending_cas_rw) {
    ...
    if (*((lock_t*)re.cas_buf) != STATE_CLEAN) {
      ...
      auto remote_lock_addr = re.item->item_ptr->GetRemoteLockAddr(remote_data_addr);
       ...
       uint64_t last_fuckedup = 1000;

      while (*((lock_t*)re.cas_buf) != STATE_CLEAN) {

	      if(*((lock_t*)re.cas_buf) != last_fuckedup){

			RDMA_LOG(INFO) << "Thread " << t_id << " Lock FUCKED " << *((lock_t*)re.cas_buf);
	      		last_fuckedup = *((lock_t*)re.cas_buf);
	      }
        // timing
        Timer timer;
        timer.Start();
   ...
   RDMA_LOG(INFO) << "Thread " << t_id << " Lock Suceessful"; 
    ...
    if (likely(fetched_item->key == it->key && fetched_item->table_id == it->table_id)) {
	  ...
	  RDMA_LOG(INFO)<< "No insert fetched lock and version key and table " << it->lock << " " << it->version << " " << it->key;
        } else {
