...
//Specific Thread and corotine for coordinators. 
...
//DAM - For Latch recovery for all pending transactions.
bool DTX::IssueLatchLogRecoveryReadForAllThreads(coro_yield_t& yield, AddrCache  ** addr_caches, t_id_t failed_thread_id, coro_id_t failed_coro_id){
    ...
    for (int c = 1 ; c < num_coro ; c++){   
		...
		RDMA_LOG(INFO) << "Record Thread "<< t  <<" , coro " << c << " , record "<< r << " of node " << i << " : tx_id " << record[r].tx_id_ << " , tid  " << record[r].t_id_ << " Key " << record[r].key_ << " Table Id " << record[r].table_id_;
		if(record[r].tx_id_ <= 0){
                    if(r==0){
                        //not even started
                        log_received &= false;  
            ...
            if(!tx_id_agreed){
                ...
                //has_started_commit = false;
                //break;
		RDMA_LOG(INFO)<<"Tx Id NOT aggreded ";
                //tight implementation where all locks of the last transaction is set regardless of the last flagged. 
                if(r==0){
                    has_started_commit = false;
                }
    		...
    		RDMA_LOG(INFO)<<"Tx Id agreed ";
                ...
                if(!last_flagged){
		RDMA_LOG(INFO)<<"Not last flagged ";
                    //continue with the next log record if if
                    assert((r+1) <= MAX_LATCH_LOG_RECORDS);
                    //continue;
                }else{

			RDMA_LOG(INFO)<<"Last Flagged ";
                    //then only we need to read undo
                    //if last flagged, tx ids agrred and last tx_id is not complete. then only undo logs are needed. 
		    ...
		    RDMA_LOG(WARNING)<< "Unlocking key " << logged_key << " of table  " << logged_table_id;
                    //RDMACAS(coro_id_t coro_id, RCQP* qp, char* local_buf, uint64_t remote_offset, uint64_t compare, uint64_t swap)
                    //TODO - STATE_LOCKED must be s
                    #ifdef ELOG
		     bool state=coro_sched->RDMACAS(coro_id, qp, cas_buf, offset, (t+1), STATE_CLEAN);
		     RDMA_LOG(WARNING) << " Thread Id " << t+1;
                        if (!state) {
                    #else
                        if (!coro_sched->RDMACAS(coro_id, qp, cas_buf, offset, STATE_LOCKED, STATE_CLEAN)) {
                    #endif
		    //if (!coro_sched->RDMACAS(coro_id, qp, cas_buf, offset, STATE_LOCKED, STATE_CLEAN)) {
                       ...
                       //TODO- report back to me
                       RDMA_LOG(WARNING)<< "offset not founf";
                        //NEW - just keep continuing
                       //assert(false);
                       //report  back issues.
                   }            
        ...
        for (int c = 1 ; c < num_coro ; c++){   
            ...
            for(int r=0; r < MAX_DATA_LOG_RECORDS; r++ ){    
                ...
                for (int i = 0; i < global_meta_man->remote_nodes.size(); i++){      
                    ...
                    //FIX- check if the curretn agrred tx id is.
        		RDMA_LOG(INFO) << "Record Thread "<< t  <<" , coro " << c << " , record "<< r << " of node " << i << " : tx_id " << record[r].tx_id_ << " , tid  " << record[r].t_id_ << " Key " << record[r].data_.key << " Table Id " << record[r].data_.table_id;
		    ...
		    if(record[r].tx_id_ <= 0){
                ...
                if(!tx_id_agreed){
		    RDMA_LOG (INFO) << "chk 3 tx id not aggrreed";
                    // simply unlock places and continue. 
                    //coro_agreed_tx_id = curr_tx_id;
                ...
                //all last tx_ids agreed. 
               		...
               		RDMA_LOG(INFO) << "chk 3 Thread " << t << " , Coro " << c << " , has started commited Tx " << curr_agreed_tx_id << " curr " << coro_agreed_tx_id << " valid logs " << num_valid_logs;	     
                    if(curr_agreed_tx_id < coro_agreed_tx_id){
                        //canot be larger. assert. we write logs in order. cannt get reordered.
                        has_started_commit = true;
                        num_valid_logs = r;  // remove the last log recored. which is old.
            ...
            if(!has_started_commit){
               coro_has_started_commit[t][c] = has_started_commit; ; //TODO- iinvoke unlocks
	       RDMA_LOG(WARNING) << "Thread " << t << " , Coro " << c << " , has  not started commited Tx " << coro_agreed_tx_id;
            }
                ...
                //remember. last flagged only workd in with inserts.
                //RDMA_LOG(FATAL) << "Thread " << t << " , Coro " << c << " , has started commited Tx " << coro_agreed_tx_id; 
     		RDMA_LOG(WARNING) << "Thread " << t << " , Coro " << c << " , has started commited Tx " << coro_agreed_tx_id; 
                //Iterate through logs (table, key) and read in-place values.
