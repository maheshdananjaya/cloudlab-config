...
bool Litmus3Alt_T1(coro_yield_t& yield, tx_id_t tx_id, DTX* dtx) {  
 //No more random writes. pick any combinations. 
  //micro_key.item_key = (itemkey_t)FastRand(&seed) & (num_keys_global - 1);
  //
  RDMA_LOG(WARNING) << "Starting Litmus 3 T1 thread " << dtx->t_id << " coro " << dtx->coro_id; 
  dtx->TxBegin(tx_id);
  bool is_write[data_set_size]; 
  DataItemPtr micro_objs[data_set_size];
   ...
   //DelayRandom();
    ...
    if(rand_num==1){
      ...
      RDMA_LOG(INFO) << "Litmus 3 alt t1 values before abort/fail Litmus3Alt_T1 -  X= " << micro_val_x->magic[1] << " , Y= " <<  micro_val_y->magic[1];
      RDMA_LOG(INFO) << " Litmus 3 alt t1: Recovery start thread_id= " << thread_gid << " coro_id=" << dtx->coro_id;
      dtx->TxUndoRecovery(yield, addr_caches, thread_gid, dtx->coro_id); //NEW recovery with global coordinator id. 
      RDMA_LOG(INFO) << " Litmus 3 alt t1: Lock Recovery start thread_id= " << thread_gid << " coro_id=" << dtx->coro_id;
    ...
    //CRASH points 4
    if(commit_status)
      RDMA_LOG(INFO) << "Litmus3Alt_T1 Commited  -  X= " << value_v+1 << " , Y= " <<  value_v+1;
...
bool Litmus3Alt_T2(coro_yield_t& yield, tx_id_t tx_id, DTX* dtx) {
   ...
   //No more random writes. pick any combinations. 
  //micro_key.item_key = (itemkey_t)FastRand(&seed) & (num_keys_global - 1);
  RDMA_LOG(WARNING) << "Starting Litmus 3 T2 thread " << dtx->t_id << " coro " << dtx->coro_id;
  dtx->TxBegin(tx_id);
  bool is_write[data_set_size]; 
  DataItemPtr micro_objs[data_set_size];
...
bool Assert3Alt_T2(coro_yield_t& yield, tx_id_t tx_id, DTX* dtx) {
  ...
  //assertion function for the mitmus testing
  //trick: either stop the world or lock all read only objects.
  RDMA_LOG(WARNING) << "Starting Assert 3 T2 thread " << dtx->t_id << " coro " << dtx->coro_id;
  dtx->TxBegin(tx_id);
  bool is_write[data_set_size]; 
  DataItemPtr micro_objs[data_set_size];
    ...
    if (!dtx->TxExeLitmus(yield)) {
      ...
      //RDMA_LOG(INFO) << "Before Litmus3AltT1_Assert -  X= " << micro_val_x->magic[1] << " , Z = " <<  micro_val_z->magic[1];
       RDMA_LOG(INFO) << "Before Litmus3Alt_T2_Assert - thread id= "<< thread_gid << " coro= " << dtx->coro_id <<   " X= " << micro_val_x->magic[1] << " , Z = " <<  micro_val_z->magic[1];
...
bool Assert3Alt_T1(coro_yield_t& yield, tx_id_t tx_id, DTX* dtx) {
  ...
  //assertion function for the mitmus testing
  //trick: either stop the world or lock all read only objects.
  RDMA_LOG(WARNING) << "Starting Assert 3 T1 thread " << dtx->t_id << " coro " << dtx->coro_id;  
  dtx->TxBegin(tx_id);
  bool is_write[data_set_size]; 
  DataItemPtr micro_objs[data_set_size];
    ...
    if (!dtx->TxExeLitmus(yield)) {
      ...
      RDMA_LOG(INFO) << "Before Litmus3Alt_T1_Assert -  X= " << micro_val_x->magic[1] << " , Y = " <<  micro_val_y->magic[1];
